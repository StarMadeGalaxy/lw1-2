...
int64_t Expression::evaluate_postfix(Stack** stack_pointer)
{
	Stack stack;

	std::vector<int64_t> variable_values(0);
	...
	for (size_t i = 0; i < postfix.size(); i++)
	{
		...
		if (is_operand(current_symbol)) {
			...
			int64_t first_value = stack.pop(stack_pointer);
			int64_t second_value = stack.pop(stack_pointer);
